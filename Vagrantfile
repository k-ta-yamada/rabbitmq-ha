...
Vagrant.configure("2") do |config|
  ...
  config.vm.define :mq1 do |mq1|
    mq1.vm.provider "virtualbox" do |v|
      v.linked_clone = true
    end

    mq1.vm.hostname = 'mq1'

    mq1.vm.network "private_network", ip: "192.168.1.101"
    mq1.vm.network "private_network", ip: "192.168.2.101"

    mq1.vm.provision "ssh:root", type: :shell do |s|
      private_key = "/vagrant/.vagrant/machines/#{mq1.vm.hostname}/virtualbox/private_key"
      s.inline = <<-SHELL
  ...
  config.vm.define :mq2 do |mq2|
    mq2.vm.provider "virtualbox" do |v|
      v.linked_clone = true
    end

    mq2.vm.hostname = 'mq2'

    mq2.vm.network "private_network", ip: "192.168.1.102"
    mq2.vm.network "private_network", ip: "192.168.2.102"

    mq2.vm.provision "ssh:root", type: :shell do |s|
      private_key = "/vagrant/.vagrant/machines/#{mq2.vm.hostname}/virtualbox/private_key"
      s.inline = <<-SHELL
        sudo su - root -c "mkdir -p /root/.ssh/"
        sudo chmod 700 /root/.ssh/
        ssh-keygen -yf #{private_key} > /root/.ssh/authorized_keys
